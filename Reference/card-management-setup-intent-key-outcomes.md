...
- **Implementation shape (Faruk, agreed 2026-07-21):** I spec tightly, then dispatch named sub-agents to implement granular parts with model matched to complexity (Opus = hairy parts, Sonnet = routine, Haiku = test scaffolding); I review/verify with a separate agent. Local-only changes — commits/pushes/PR actions stay with Ankur (GitHub read-only rule).
- **Full agreed pipeline (Faruk, 2026-07-21):** discover (Serena code walk incl. ~#406 expired-card fingerprint fix + Context7 Stripe docs on setup intents/mandates/payment methods) → enumerate case matrix → brainstorming skill to refine design → tight spec → delegate by complexity → separate-agent review. Trigger: Ankur shares the DTO + mockups.
